...
class Defense {
  constructor(data = {}) {
    ...
    this.range = T.range;
    this.worldRangeMultiplier = 1;
    this.fireRate = T.fireRate;
    this.rarity = T.rarity || "Common";
    this.slowAmount = Number(T.slow) || 0;
  ...
  _buildModel(T) {
    ...
    const turretGeometry =
      new THREE.SphereGeometry(
        this.typeId === "storm_battery" ? 0.38 : 0.32,
        16,
        12
      );
    ...
    const ring = new THREE.Mesh(
      new THREE.RingGeometry(0.38, 0.43, 24),
      new THREE.MeshStandardMaterial({ color: T.projectileColor, transparent: true, opacity: 0.72, emissive: T.projectileColor, emissiveIntensity: 0.35 })
    );
  ...
  _isTargetValid(enemy) {
    ...
    return (
      ...
      ) <= (this.range * this.worldRangeMultiplier)
    );
  }
  ...
  _fire(enemy) {
    ...
    ProjectileManager.spawn({
      ...
      status: this.slowAmount > 0 ? { type: "slow", value: this.slowAmount, duration: 1.6 } : null,
    });
  }
